...
wa_connector.getSchema = function(request) {
  if (!token) {
    token = getAccessToken(request.configParams.apikey);
    // token = getAccessToken(request.key);
  }
...
function fetchAPI(url, token) {
  ...
  try {
    var responseJSON = UrlFetchApp.fetch(url, {
      method: "GET",
      headers: { Authorization: "Bearer " + token, "User-Agent": "Pro Reports for Wild Apricot (WARM) / 2.1.2" },
...
/////////////////////////// NEW AUTH APPROACH REPLACING getConfig() /////////////////////////////////
/**
 * Gets the APIKEY Auth type.
...
wa_connector.getAuthType = function() {
  ...
  return cc
    ...
    .setHelpUrl("https://gethelp.wildapricot.com/en/articles/180-authorizing-external-applications")
    .build();
};
...
function setCredentials(request) {
  ...
  // You would have to have a `getAccessToken` function defined for
  // this to work.
  var validKey = getAccessToken(key);
  if (!validKey) {
    return {
